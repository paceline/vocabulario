class ScoresController < ApplicationController
  ...
  def create
    if params[:test][:from] != params[:test][:to]
      ...
      flash[:error] = "It's a vocabulary test. You probably want to select two different languages."
      render :update do |page|
  ...
  def update
    ...
    if session[:test].result_for(params[:test][:answer])
      ...
      flash.now[:error] = "Unfortunately that's not correct. #{session[:test].from.word} - #{session[:test].correct_results.join(', ')}."
    end
